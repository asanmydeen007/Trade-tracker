    ...
    <div className="card p-4 space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-sm font-semibold">
            {data.name} {isGold ? "24K" : ""}{" "}
            <span className="text-xs font-normal text-slate-500">({data.symbol})</span>
          </div>
          <div className="text-[11px] text-slate-500 mt-0.5">India-focused · live international feed</div>
        </div>
        <div
      ...
      <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4">
        ...
        <div className="mt-1 flex flex-wrap items-baseline gap-3">
          <div className="text-2xl font-bold text-slate-900 dark:text-white">
            ₹{fmt(data.perGramInrIndia, 0)}
          </div>
          <div className="text-sm text-slate-500">${fmt(data.perGramUsd, 2)} / g</div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-xl bg-slate-100 dark:bg-slate-800/60 p-3">
          <div className="text-[10px] uppercase tracking-wide text-slate-500">Intl spot / oz</div>
          <div className="font-semibold mt-0.5">${fmt(data.priceUsd)}</div>
          <div className="text-[11px] text-slate-500">₹{fmt(data.priceInr, 0)}</div>
        </div>
        <div className="rounded-xl bg-slate-100 dark:bg-slate-800/60 p-3">
          <div className="text-[10px] uppercase tracking-wide text-slate-500">Spot 1g (no premium)</div>
          <div className="font-semibold mt-0.5">₹{fmt(data.perGramInr, 0)}</div>
          <div className="text-[11px] text-slate-500">${fmt(data.perGramUsd, 2)}</div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 p-3">
        ...
        <div className="mt-2 text-[10px] text-slate-400">
          Range position: {data.insight.rangePositionPct}% from day low → high (oz levels)
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div>
          <div className="text-sm font-semibold">Gold & Silver (India)</div>
          <div className="text-[11px] text-slate-500">
            Per gram INR · 24h change · S/R insight
      ...
      {data?.asOf && (
        <div className="text-[11px] text-slate-500">
          Updated {new Date(data.asOf).toLocaleString("en-IN")} · Feed: GC=F / SI=F + USDINR
        </div>
      )}
    </div>
  );
